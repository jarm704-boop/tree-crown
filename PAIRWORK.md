...
- **Hayden:** contributed to early discussions on what the new dataset would need to provide.
...
- **Hayden:** wrote the initial API call function that `_read_arcgis_layer()` is based on, contributed to the poster design, and helped with the poster showcase.
- **Josh** was the primary code contributor for this phase:
  ...
  - `demo_treecrown_nz.ipynb`: a demonstration notebook covering suburb ranking, canopy mapping, and walking shade analysis.
...
Finding a suitable dataset was the group's biggest challenge. Once we landed on the Wellington City Council canopy data, things moved along pretty smoothly. We communicated well throughout the project and got tasks done in good time. The final package came out better than expected: it does what we set out to do, it is well documented, and it has been properly tested.

## Aspects to improve

The coding was not evenly split across the group as Josh did the bulk of the implementation, while Joel and Hayden contributed more on the documentation and design side. The main reason for this was the time pressure we faced after the Auckland dataset was not feasible. We had no working code to build from without the dataset and needed to move quickly before the presentation date within a few days, so it made sense to lean on whoever could get things done fastest. In an ideal situation, everyone would have had more of a hand in the code itself.
